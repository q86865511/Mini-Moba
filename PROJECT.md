...
- 2026-06-04：完成階段 1。建立 `shared` 純模擬庫（`Vec2` / `Hero` / `World::Tick` 朝目標移動，零 raylib／零網路）；client 連結 shared，固定時間步長驅動，右鍵點地面移動、畫格線地圖。期間踩到 Smart App Control（強制開啟）封鎖未簽署 exe，使用者關閉 SAC 後正常。
